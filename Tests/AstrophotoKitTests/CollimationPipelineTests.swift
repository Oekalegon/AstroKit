...
func testDonutRadiiConsistency() async throws {
    ...
    guard let df, df.rows.count >= 2 else {
        print("⚠️  Fewer than 2 donuts detected — skipping consistency check")
        return
    }
    ...
    print("Outer radii: mean=\(String(format:"%.1f",mean))px stddev=\(String(format:"%.1f",stddev))px CV=\(String(format:"%.3f",cv))")

    // All donuts share the same telescope optics so radii should be consistent (CV < 10%)
